...
// --- [PERBAIKAN BUG 1: Pindahkan normalizeMembers ke scope global file] ---
/**
 * Menormalisasi data member dari API CoC yang tidak konsisten.
 * API CWL mengirim 'townHallLevel', API War Klasik mengirim 'townhallLevel'.
 * Tipe internal kita (CocWarMember) menggunakan 'townhallLevel' (h kecil).
 * @param member Member dari API (tipe 'any' untuk menangani properti dinamis)
 * @returns {CocWarMember} Member yang sudah dinormalisasi
 */
const normalizeMembers = (member: any): CocWarMember => ({
  ...member,
  // Ambil 'townHallLevel' (H besar), fallback ke 'townhallLevel' (h kecil), lalu 0
 ...
 * @returns {Promise<CocCurrentWar | null>} Data perang aktif atau null (SUDAH DINORMALISASI)
 */
export async function getClanCurrentWar(
  ...
  try {
    ...
    const warResponse = await fetchCocApi<
      CocCurrentWar | { reason?: string; state?: string }
    >(`/clans/${encodedClanTag}/currentwar`);

    // 2. Jika perang reguler ditemukan dan aktif, normalisasi dan kembalikan
    if (
      'state' in warResponse &&
      (warResponse.state === 'inWar' ||
        warResponse.state === 'preparation' ||
        // [PERBAIKAN BUG 4] Kita juga ingin menangkap state 'warEnded'
        warResponse.state === 'warEnded')
    ) {
      console.log(
        `[getClanCurrentWar] Found active/ended regular war for ${rawClanTag}. State: ${warResponse.state}`
      );

      // --- [PERBAIKAN BUG 1: Terapkan Normalisasi di sini] ---
      // API Perang Klasik (/currentwar) TIDAK konsisten dan
      // sering mengirim 'townHallLevel' (H besar). Kita normalisasi ke 'townhallLevel' (h kecil).
      const fixedWarDetail: CocCurrentWar = {
        ...(warResponse as CocCurrentWar),
        clan: {
          ...(warResponse as CocCurrentWar).clan,
          members: (warResponse as CocCurrentWar).clan.members.map(
            normalizeMembers
          ),
        },
        opponent: {
          ...(warResponse as CocCurrentWar).opponent,
          members: (warResponse as CocCurrentWar).opponent.members.map(
            normalizeMembers
          ),
        },
      };
    ...
    if (
      ...
      try {
        ...
        // [PERBAIKAN BUG 4] Kita juga ingin menangkap 'warEnded' dari CWL
        if (
          cwlGroup.state !== 'inWar' &&
          cwlGroup.state !== 'preparation' &&
          cwlGroup.state !== 'ended' // 'ended' berarti CWL selesai
        ) {
          console.log(
            `[getClanCurrentWar] CWL Group is state: ${cwlGroup.state}. Clan not in active war.`
          );
          return null;
        }
        ...
        if (!allWarTags || allWarTags.length === 0) {
          console.log(
            `[getClanCurrentWar] CWL group active, but no war tags found.`
          );
          return null;
        }
        ...
        console.log(
          `[getClanCurrentWar] Fetching details for ${allWarTags.length} CWL wars...`
        );
        const warDetailPromises = allWarTags.map((tag) =>
          tag !== '#0'
            ? getLeagueWarDetails(encodeURIComponent(tag))
        ...
        for (const result of results) {
          if (result.status === 'fulfilled' && result.value) {
            ...
            // [PERBAIKAN BUG 4] Kita juga cek 'warEnded' untuk CWL
            const isWarActiveOrEnded =
              warDetail.state === 'inWar' ||
              warDetail.state === 'preparation' ||
              warDetail.state === 'warEnded';

            if (isClanInWar && isWarActiveOrEnded) {
              console.log(
                `[getClanCurrentWar] Found active/ended CWL war: ${warDetail.clan.name} vs ${warDetail.opponent.name}`
              );

              // --- [PERBAIKAN UI BERANTAKAN] ---
              // Logika normalisasi Anda sudah ada di sini, kita pastikan tetap ada.
              // Tipe 'any' digunakan di sini secara sengaja untuk menangani data mentah
              // dari API yang tidak konsisten
              
              // (Fungsi normalizeMembers sekarang ada di scope global file)
        ...
        console.log(
          `[getClanCurrentWar] CWL group active, but no matching active/ended war found for clan ${rawClanTag}.`
        );
        return null;
      } catch (cwlError) {
        if (cwlError instanceof Error && cwlError.message.startsWith('notFound')) {
          ...
          console.error(
            `[getClanCurrentWar] Error fetching CWL group for ${rawClanTag}:`,
            cwlError
          );
        }
    ...
    console.warn(
      `[getClanCurrentWar] Unknown response from /currentwar for ${rawClanTag}:`,
      warResponse
    );
    return null;
  } catch (error) {
    ...
    if (error instanceof Error && error.message.startsWith('notFound')) {
      console.warn(
        `[getClanCurrentWar] API check failed: Clan ${rawClanTag} not found (404).`
      );
    } else {
      console.error(
        `[getClanCurrentWar] Error checking current war for ${rawClanTag}:`,
        error
      );
    }
