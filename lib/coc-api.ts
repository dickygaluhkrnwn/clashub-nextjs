...
// PERBAIKAN: Impor CocCurrentWar
import { CocClan, CocMember, CocPlayer, CocWarLog, CocCurrentWar } from './types';
...
// PERBAIKAN: Mengubah return type Promise dari CocWarLog ke CocCurrentWar
export async function getClanCurrentWar(encodedClanTag: string, rawClanTag: string): Promise<CocCurrentWar | null> {
    try {
        // PERBAIKAN: Mengubah Tipe Generic fetchCocApi ke CocCurrentWar
        const warResponse = await fetchCocApi<CocCurrentWar | { reason: string }>(`/clans/${encodedClanTag}/currentwar`);

        if ('state' in warResponse && (warResponse.state === 'inWar' || warResponse.state === 'preparation')) {
            // PERBAIKAN: Assertion ke CocCurrentWar
...
                    if (currentWarTag) {
                        // PERBAIKAN: Endpoint CWL War juga mengembalikan tipe CocCurrentWar
                        return fetchCocApi<CocCurrentWar>(`/clanwarleagues/wars/${encodeURIComponent(currentWarTag)}`);
}
                }
            } catch (cwlError) {
