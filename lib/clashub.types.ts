 ...
 * [BARU: Rombak Fitur Turnamen]
 * Menggantikan data dummy. Ini adalah struktur data utama untuk sebuah turnamen.
 * Disimpan di koleksi root /tournaments
 */
export interface Tournament {
  id: string; // ID dokumen Firestore

  // Info Utama
  title: string; // [DIUBAH] dari 'name' agar konsisten dengan TournamentCard
  description: string;
  rules: string;
  bannerUrl: string; // URL ke Firebase Storage
  status: 'UPCOMING' | 'ONGOING' | 'COMPLETED'; // Sesuai roadmap

  // Info Organizer
  organizerId: string; // UID dari UserProfile pembuat
  organizerName: string; // Nama display pembuat

  // Detail Event
  startDate: Date; // Menggunakan tipe Date, sesuai konvensi file ini
  prizePool: string;
  format: string;
  teamSize: number;
  maxParticipants: number;
  thRequirement: string; // [BARU] Ditambahkan untuk filter TH (Cth: "TH 14 - 16" atau "TH 16 Only")

  // Metadata
  createdAt: Date;
  participantCount: number; // Denormalisasi jumlah peserta (tim)
}

/**
 * @interface TournamentParticipant
 * [BARU: Rombak Fitur Turnamen]
 * Struktur data untuk tim/clan yang mendaftar turnamen.
 * Disimpan di sub-koleksi /tournaments/{tournamentId}/participants/{participantId}
 */
export interface TournamentParticipant {
  id: string; // ID Dokumen (bisa clanTag atau UID pendaftar)
  clanTag: string;
  clanName: string;
  clanBadgeUrl: string; // URL badge (dari CocIconUrls.medium)
  representativeId: string; // UID UserProfile yang mendaftarkan
  representativeName: string; // Nama display pendaftar
 ...
 * Struktur data untuk ulasan yang diberikan kepada sebuah klan.
 * Disimpan di koleksi root 'clanReviews'.
 */
 ...
 * Struktur data untuk ulasan yang diberikan kepada seorang pemain.
 * Disimpan di koleksi root 'playerReviews'.
 */
