...
export const getPosts = async (
    ...
    // (Jika 'likes', kita sort by 'likes'. Jika 'createdAt', kita sort by 'createdAt'.)
    // (Video akan di-sort by 'publishedAt' secara terpisah)
    ...
    if (shouldFetchPosts) {
        ...
        // Jika 'all' atau 'all-posts', jangan filter kategori (ambil semua post).
        ...
        queries.push(
            clientGetDocs(qPosts).then(snapshot => {
                snapshot.docs.forEach(doc => {
                    ...
                    // 6. PERBAIKAN: Jika kategori 'all-posts', JANGAN tambahkan post 'Berita Komunitas' (opsional, tapi lebih bersih)
                    // if (post && category === 'all-posts' && post.category === 'Berita Komunitas') {
                    //     return; // Lewati post 'Berita Komunitas' jika kita hanya ingin 'Semua Diskusi'
                    // }
                    // 7. PERBAIKAN: Jika kategori 'all-posts', jangan tambahkan video (sudah ditangani di video query)
                    // Logika di atas (baris 619) sudah benar.
        ...
        queries.push(
            clientGetDocs(qVideos).then(snapshot => {
                snapshot.docs.forEach(doc => {
                    ...
                    // PERBAIKAN ERROR TS2367: Hapus perbandingan category !== 'all-posts'
                    if (video) {
                        results.push(video);
                    }
                });
