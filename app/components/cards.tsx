    ...
    return (
        <div className="card-stone flex flex-col justify-between h-full p-5 transition-transform hover:scale-[1.02] duration-300">
            <div>
                <div className="flex items-start gap-4 mb-4 border-b border-coc-gold-dark/20 pb-4">
                    ...
                    <div className="flex-grow min-w-0">
                        ...
                        <h4 className="font-clash text-xl text-white leading-tight truncate">{name}</h4>
            ...
            <Link href={`/clan/internal/${id}`} className="mt-5">
                <Button variant="secondary" className="w-full">Lihat Profil Tim</Button>
            </Link>
        </div>
    );
};
            ...
            <div className="card-stone h-full flex flex-col p-5 hover:bg-coc-stone-light/90 transition-colors duration-200">
                <div className="flex-grow">
                    <div className="flex items-center gap-2 mb-3 text-xs flex-wrap">
                        ...
                        <span className="px-2 py-1 font-semibold bg-coc-stone-light text-coc-gold rounded-sm border border-coc-gold-dark/30">{tag}</span>
                    </div>
                    {/* Menambahkan font-clash */}
...
export const TournamentCard = ({ id, title, status, thRequirement, prizePool }: TournamentCardProps) => {
    ...
    return (
        <div className={`card-stone flex flex-col sm:flex-row justify-between items-center p-6 gap-4 border-l-4 ${statusStyles[status]} transition-shadow hover:shadow-xl rounded-lg`}>
            ...
            <div className="flex flex-col items-stretch sm:items-end gap-3 w-full sm:w-auto mt-4 sm:mt-0">
                <span className={`px-3 py-1 text-xs font-bold rounded-full text-center font-sans ${statusBadgeStyles[status]}`}>{status}</span>
            ...
            <div>
                ...
                <div className="space-y-3 pt-4 font-sans">
                    <div className="flex justify-between items-center text-sm">
                        ...
                        <span className={`px-3 py-1 text-xs font-bold rounded-full font-sans ${roleColors[role] || 'bg-gray-600'}`}>{role}</span>
                    </div>
                    <div className="flex justify-between items-center text-sm">
                        <span className="font-bold text-gray-300">Town Hall:</span>
                        <span className="font-bold text-white font-clash text-lg">TH {thLevel}</span>
                    </div>
                    <div className="flex justify-between items-center text-sm">
