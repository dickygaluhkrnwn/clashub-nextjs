            ...
            <div>
                <div className="flex items-start gap-4 mb-4 border-b border-coc-gold-dark/20 pb-4">
                    ...
                    <div className="flex-grow min-w-0">
                        <h4 className="font-clash text-xl text-white leading-tight truncate">{clan.name}</h4>
                ...
                <div className="space-y-3 pt-4 font-sans text-sm">
                    ...
                    <div className="flex justify-between items-center">
                        ...
                        <span className="font-bold text-coc-green capitalize">{clan.type || 'N/A'}</span>
                    </div>
                    <div className="flex justify-between items-center">
                        <span className="font-bold text-gray-300">Poin Klan:</span>
                        <span className="font-bold text-white">{clan.clanPoints?.toLocaleString() || 'N/A'}</span>
                    </div>
                </div>
            </div>
            {/* Tombol ke halaman Clan Publik */}
            <Link href={cocProfileUrl} className="mt-5">
                <Button variant="primary" className="w-full">Lihat Profil CoC</Button>
            </Link>
        </div>
    );
};
...
const TeamHubClient = ({ initialClans, initialPlayers, initialPublicClans }: TeamHubClientProps) => {
    ...
    const filteredClans = useMemo(() => {
        return allClans.filter((clan: ManagedClan) => {
    ...
    const filteredPlayers = useMemo(() => {
        return allPlayers.filter((player: Player) => { // FIX TS7006
