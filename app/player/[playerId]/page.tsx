...
import {
  ...
  getUserProfileByTag,
  getPostsByAuthor,
} from '@/lib/firestore'; // <<<--- IMPORT BARU: getPostsByAuthor
...
const PlayerDetailPage = async ({ params }: PlayerDetailPageProps) => {
  ...
  const validThLevel =
    player.thLevel && !isNaN(player.thLevel) && player.thLevel > 0
      ? player.thLevel
      : 9;
  ...
  const isCompetitiveVision =
    player.playStyle === 'Attacker Utama' ||
    player.playStyle === 'Strategist';

  // Mendapatkan data klan internal jika pemain tergabung (ManagedClan)
  let managedClanData: ManagedClan | null = null;
  // [FIX] Mengganti player.teamId menjadi player.clanId
  if (player.clanId) {
    managedClanData = await getManagedClanData(player.clanId);
  }
  ...
  const currentTeamHistory = managedClanData
    ? [
        {
          // [FIX] Mengganti player.teamName menjadi player.clanName
          name: player.clanName || managedClanData.name,
      ...
      <section className="grid grid-cols-1 lg:grid-cols-4 gap-8">
        ...
        <aside className="lg:col-span-1 card-stone p-6 h-fit sticky top-28 space-y-6 text-center rounded-lg">
          ...
          <p className="text-sm text-gray-400 font-bold mb-1 font-mono">
            {player.playerTag || 'TAG TIDAK TERSEDIA'}
          </p>
          {/* Status Free Agent */}
          ...
          <div className="text-left pt-4 border-t border-coc-gold-dark/20 space-y-4">
            ...
            <p className="text-sm text-gray-300">
              {player.bio || 'Pemain ini belum melengkapi bio-nya.'}
            </p>
          </div>
          <div className="text-left pt-4 border-t border-coc-gold-dark/20 space-y-4">
            <h3 className="text-lg text-coc-gold-dark font-clash flex items-center gap-2">
            ...
            <p className="text-sm">
              ...
              {player.playStyle || 'Belum Diatur'}
            </p>
            <p className="text-sm">
              <span className="font-bold text-gray-300">Jam Aktif:</span>{' '}
              {player.activeHours || 'Belum Diatur'}
            </p>
          </div>
          {/* Kontak Sosial */}
          <div className="text-left pt-4 border-t border-coc-gold-dark/20 space-y-2">
            ...
            {player.discordId ? (
              <p className="text-sm text-gray-300 flex items-center gap-2">
                ...
                <span className="font-bold">{player.discordId}</span>
              </p>
            ) : (
            ...
            {player.website ? (
              <a
                href={
                  player.website.startsWith('http')
                    ? player.website
                    : `https://${player.website}`
                }
                target="_blank"
              ...
              >
                ...
                {player.website.replace(/^(https?:\/\/)?(www\.)?/, '').split('/')[0]}
              </a>
            ) : (
              <p className="text-sm text-gray-500 flex items-center gap-2">
          ...
          <div className="pt-4 border-t border-coc-gold-dark/20 text-center">
            ...
            <p className="text-4xl font-clash text-coc-gold my-1">
              {player.reputation ? player.reputation.toFixed(1) : '5.0'}{' '}
              <StarIcon className="inline h-7 w-7" />
            </p>
            <p className="text-xs text-gray-400">(8 Ulasan Komitmen)</p>
            <Button
              href={`/player/${player.uid}/reviews`}
              variant="secondary"
              className="w-full mt-4"
            >
              Lihat Semua Ulasan
            </Button>{' '}
            {/* Menggunakan player.uid yang dijamin ada */}
          </div>
        </aside>
        ...
        <section className="lg:col-span-3 space-y-8">
          ...
          <div className="card-stone p-6 rounded-lg">
            <h2 className="mb-6 flex items-center gap-2 font-clash text-2xl text-white">
              ...
              {isVerified ? '(Terverifikasi CoC)' : '(Belum Diverifikasi)'}
            </h2>
            ...
            <div className="flex flex-col md:flex-row items-center gap-6">
              ...
              <div className="flex-grow grid grid-cols-2 gap-4 text-center w-full">
                ...
                <div className="bg-coc-stone/50 p-4 rounded-lg border border-coc-gold-dark/30">
                  <h4 className="text-3xl text-coc-gold font-clash">
                    {player.trophies?.toLocaleString() || 'N/A'}
                  </h4>
                  <p className="text-xs uppercase text-gray-400 font-sans">
                    Trofi Saat Ini
                  </p>
                ...
                <div className="bg-coc-stone/50 p-4 rounded-lg border border-coc-gold-dark/30">
                  ...
                  {/* Placeholder */}
                  <p className="text-xs uppercase text-gray-400 font-sans">
                    Bintang War Diperoleh
                  </p>
                </div>
                <div className="bg-coc-stone/50 p-4 rounded-lg border border-coc-gold-dark/30">
                  <h4 className="text-lg text-coc-gold font-clash capitalize">
                    {isVerified ? (inGameRole || 'N/A') : 'N/A'}
                  </h4>
                  <p className="xs uppercase text-gray-400 font-sans">
                    Role di Klan CoC
                  </p>
                </div>
                ...
                {isVerified && player.clanTag && (
                  <div className="bg-coc-stone/50 p-4 rounded-lg border border-coc-gold-dark/30 col-span-2">
                    <Link
                      href={`/clan/${encodeURIComponent(player.clanTag)}`}
                      className="hover:opacity-80 transition-opacity"
                    >
                      <h4 className="lg text-coc-gold font-mono">
                        {player.clanTag}
                      </h4>
                      <p className="xs uppercase text-gray-400 font-sans">
                        Klan CoC Saat Ini
          ...
          <div className="card-stone p-6 rounded-lg">
            <h2 className="mb-4 font-clash text-2xl text-white">
              Aktivitas & Postingan Terbaru
            </h2>
              ...
              {recentPosts.length > 0 ? (
                ...
                <div className="text-center py-4">
                  <p className="text-gray-400">
                    Pemain ini belum memposting di Knowledge Hub.
                  </p>
          ...
          <div className="card-stone p-6 rounded-lg">
            <h2 className="mb-4 flex items-center gap-2 font-clash text-2xl text-white">
              <ShieldCheckIcon className="h-6 w-6 text-coc-gold" /> Riwayat Tim
              Clashub
            </h2>{' '}
            {/* Updated Icon */}
            <div className="space-y-4">
              {currentTeamHistory.length === 0 ? (
                <p className="text-gray-400 text-sm">
                  Pemain ini adalah Free Agent dan belum memiliki riwayat tim di
                  Clashub.
                </p>
              ) : (
                currentTeamHistory.map((history, index) => (
                  <div
                    key={index}
                    className="flex flex-col p-4 bg-coc-stone/50 rounded-lg border-l-4 border-coc-gold-dark/30 hover:border-coc-gold transition-all"
                  >
                    {' '}
                    {/* Adjusted Border */}
                    <h4 className="text-lg font-clash text-white flex justify-between items-center flex-wrap gap-x-2">
                      {' '}
                      {/* Added wrap */}
                      {/* [FIX] Gunakan managedClanData.id yang mengacu ke ManagedClan ID internal */}
                      <Link
                        href={`/clan/manage?id=${history.id}`}
                        className="hover:text-coc-gold"
                      >
                        {history.name}
                      </Link>
                      <span
                        className={`px-2 py-0.5 ml-2 text-xs font-bold rounded-full whitespace-nowrap ${
                          history.role === 'Leader'
                            ? 'bg-coc-gold text-coc-stone'
                            : 'bg-gray-400 text-coc-stone'
                        }`}
                      >
                        {history.role}
                      </span>
                    ...
                    <p className="text-sm text-gray-400 mt-1">
                      Durasi: <strong className="text-coc-gold">{history.duration}</strong>{' '}
                      | Visi: {history.vision}
                    </p>
                  </div>
                ))
