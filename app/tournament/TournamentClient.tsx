...
// [FIX V2] Helper baru untuk 'TournamentCard' yang masih V1
// Mengubah ThRequirement (Objek V2) menjadi string simpel untuk UI
const formatThRequirementToString = (thReq: ThRequirement): string => {
  switch (thReq.type) {
    case 'uniform':
...
  // [FIX V2] Logika Filtering diperbarui total
  const filteredTournaments = useMemo(() => {
    return allTournaments.filter((tournament) => {
      const { status: filterStatus, thLevel, prize } = tournamentFilters;

      // 1. Filter Status (FIX: Gunakan lowercase sesuai Tipe V2)
      if (filterStatus === 'Akan Datang') {
        // [FIX V2] Status di DB sekarang 'registration_open' atau 'draft', dll.
        const isUpcoming =
          tournament.status === 'registration_open' ||
          tournament.status === 'draft' ||
          tournament.status === 'registration_closed';
        // [FIX LOGIKA ERROR] Cek langsung jika tidak match
...
      }
      // Jika filterStatus === 'Semua Status', otomatis lolos filter status

      // 2. Filter TH (FIX: Baca ThRequirement OBJECT V2)
      let thMatch = false;
      if (thLevel === 'Semua Level') {
        thMatch = true;
      } else if (tournament.thRequirement) {
        const thReq = tournament.thRequirement; // Ini object V2
        const filterIsRange = thLevel.includes(' - ');
        const filterThParts = thLevel
          .replace(/TH /g, '')
          .split(' - ')
          .map(Number);
        const filterThNum = filterThParts[0]; // Cth: 16 (dari "TH 16") atau 15 (dari "TH 15 - 17")

        if (thReq.type === 'any') {
          // Data: Rentang Min/Max (Cth: 1-17)
          if (filterIsRange) {
            // Filter 'TH 15-17' vs Data 'TH 1-17'. Cek overlap rentang
            const filterMax = filterThParts[1] || filterThParts[0];
            thMatch = thReq.minLevel <= filterMax && thReq.maxLevel >= filterThNum;
          } else {
            // Filter 'TH 16' vs Data 'TH 1-17'
            thMatch =
              filterThNum >= thReq.minLevel && filterThNum <= thReq.maxLevel;
          }
        } else if (thReq.type === 'uniform') {
          // Data: Seragam (Cth: [16])
          if (filterIsRange) {
            // Filter 'TH 15-17' vs Data 'TH 16'
            const filterMax = filterThParts[1] || filterThParts[0];
            thMatch =
              thReq.allowedLevels[0] >= filterThNum &&
              thReq.allowedLevels[0] <= filterMax;
          } else {
            // Filter 'TH 16' vs Data 'TH 16'
            thMatch = thReq.allowedLevels[0] === filterThNum;
          }
        } else if (thReq.type === 'mixed') {
          // Data: Campuran (Cth: [17,16,15,14,13])
          if (filterIsRange) {
            // Filter 'TH 15-17' vs Data [17,16,15,14,13]
            const filterMax = filterThParts[1] || filterThParts[0];
            thMatch = thReq.allowedLevels.some(
              (lvl) => lvl >= filterThNum && lvl <= filterMax,
            );
          } else {
            // Filter 'TH 16' vs Data [17,16,15,14,13]
            thMatch = thReq.allowedLevels.includes(filterThNum);
          }
        }
      }
      if (!thMatch) return false;

      // 3. Filter Hadiah (Ini sudah benar, tidak perlu diubah)
      let prizeMatch = true;
      if (prize === 'cash') {
        prizeMatch =
...
      return prizeMatch; // thMatch sudah difilter di atas
    });
...
  // [FIX V2] Helper Function for Status Translation
  // Diperbarui untuk menerima tipe status V2 (lowercase)
...
                      // [FIX V2] Gunakan helper baru untuk "menjembatani" V2 Object ke V1 String
                      thRequirement={formatThRequirementToString(
                        tournament.thRequirement,
                      )}
                      // [FIX V2] Gunakan helper yang sudah diperbarui
