...
// [EDIT TAHAP 4.2] Impor tipe data baru
import {
  UserProfile,
  Post,
  PlayerReview,
...
interface ProfileClientProps {
  ...
  recentPosts: FirestoreDocument<Post>[]; // Menerima postingan (sudah FirestoreDocument)
  // [EDIT TAHAP 4.2] Menerima props baru
...
const ProfileClient = ({
  ...
  serverError,
  recentPosts,
  // [EDIT TAHAP 4.2] Destrukturisasi props baru
  clanHistory,
  playerReviews,
}: ProfileClientProps) => {
  ...
  if (currentUser && userProfile) {
    const validThLevel =
      userProfile.thLevel && !isNaN(userProfile.thLevel) && userProfile.thLevel > 0
        ? userProfile.thLevel
        : 9;
    ...
    // [EDIT TAHAP 4.2] Menghitung total rating
    const totalRating =
      playerReviews.reduce((acc, review) => acc + review.rating, 0) /
      (playerReviews.length || 1);
    const reputation = playerReviews.length > 0 ? totalRating : 5.0; // Default 5.0 jika belum ada review
      ...
      <main className="max-w-7xl mx-auto space-y-8 p-4 md:p-8 mt-10">
        ...
        <section className="grid grid-cols-1 lg:grid-cols-4 gap-8">
          ...
          <aside className="lg:col-span-1 card-stone p-6 h-fit sticky top-28 space-y-6 text-center rounded-lg">
            ...
            <div className="text-left pt-4 border-t border-coc-gold-dark/20 space-y-2">
              ...
              {userProfile.website ? (
                ...
                <p className="text-sm text-gray-500 flex items-center gap-2">
                  <LinkIcon className="h-4 w-4 text-gray-500" /> Website belum
                  diatur
                </p>
              )}
            </div>

            {/* [EDIT TAHAP 4.2] Menampilkan Poin Popularitas */}
            ...
            <div className="pt-4 border-t border-coc-gold-dark/20 text-center">
              ...
              <p className="text-4xl font-clash text-coc-gold my-1">
                {/* [EDIT TAHAP 4.2] Menggunakan reputasi dari data review */}
                {reputation.toFixed(1)}{' '}
                <StarIcon className="inline h-7 w-7" />
              </p>
              <p className="text-xs text-gray-400">
                (Berdasarkan {playerReviews.length} ulasan)
              </p>
          ...
          <section className="lg:col-span-3 space-y-8">
                ...
                <div className="flex-grow grid grid-cols-2 gap-4 text-center w-full">
                  ...
                  {isVerified && userProfile.clanTag && (
                    <div className="bg-coc-stone/50 p-4 rounded-lg border border-coc-gold-dark/30 col-span-2">
                      {/* [EDIT TAHAP 4.2] Perbaikan Link Klan Internal */}
                      <Link
                        href={
                          userProfile.clanId
                  ...
                  <p className="text-sm font-sans text-gray-300 flex items-center gap-2">
                    ...
                    <span className="font-bold text-coc-gold">
                      {userProfile.lastVerified
                        ? new Date(userProfile.lastVerified).toLocaleString(
                            'id-ID',
                          )
                        : 'Belum Pernah'}
                    </span>
            ...
            {/* [EDIT TAHAP 4.2] Riwayat Tim (Mirip Publik) */}
            <div className="card-stone p-6 rounded-lg">
              <h2 className="mb-4 flex items-center gap-2 font-clash text-2xl text-white">
                <ShieldIcon className="h-6 w-6 text-coc-gold" /> Riwayat Tim
                Clashub
              </h2>
              <div className="space-y-4">
                {/* [EDIT TAHAP 4.2] Render data clanHistory */}
            ...
            {/* [BARU: TAHAP 4.2] Menampilkan Ulasan Diterima */}
            <div className="card-stone p-6 rounded-lg">
              <h2 className="mb-4 flex items-center gap-2 font-clash text-2xl text-white">
