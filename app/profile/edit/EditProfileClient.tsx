...
import { UserCircleIcon, SaveIcon, XIcon, InfoIcon, CheckIcon, ShieldIcon, AlertTriangleIcon, RefreshCwIcon, ArrowLeftIcon } from '@/app/components/icons';
...
const PLAY_STYLE_OPTIONS: Exclude<UserProfile['playStyle'], null | undefined>[] = ['Attacker Utama', 'Base Builder', 'Donatur', 'Strategist'];
const STATIC_AVATARS = [
    '/images/placeholder-avatar.png', '/images/barbarian.png', '/images/archer.png', '/images/giant.png', '/images/goblin.png',
    '/images/healer.png', '/images/pekka.png', '/images/wizard.png', '/images/hogrider.png', '/images/minion.png',
    '/images/valkyrie.png', '/images/witch.png', '/images/bowler.png', '/images/yeti.png', '/images/rootrider.png',
];
...
const validatePlayerTag = (tag: string): string | null => {
    ...
    if (!tagRegex.test(tag.toUpperCase())) return "Format Player Tag tidak valid (Contoh: #P9Y8Q2V).";
    return null;
};

// Reusable FormGroup Component
const FormGroup: React.FC<{ children: ReactNode; label: string; htmlFor: string; error?: string | null; disabled?: boolean }> = ({ children, label, htmlFor, error, disabled = false }) => (
    <div className="space-y-2">
        <label htmlFor={htmlFor} className={`block text-sm font-bold ${disabled ? 'text-gray-500' : 'text-gray-200'}`}>
            {label} {disabled && <span className="text-coc-red/80 font-sans text-xs">(Terkunci oleh Verifikasi)</span>}
        </label>
        {children}
...
const sanitizeDataForFirestore = (data: ProfileFormData, initialProfile: UserProfile): Partial<UserProfile> => {
...
const EditProfileClient = ({ initialProfile }: EditProfileClientProps) => {
    ...
    const handleVerificationSubmit = async () => {
        ...
        if (tagError || tokenError) {
            setNotification({ message: tagError || tokenError || "Error validasi.", type: 'error', onClose: () => setNotification(null) });
            return;
        }
        ...
        try {
            ...
            if (!response.ok) throw new Error(result.message || 'Verifikasi gagal.'); // Handle API errors

            // Show success message
            setNotification({ message: `Verifikasi sukses! Akun ${result.profile.inGameName} telah ditautkan. Halaman akan dimuat ulang.`, type: 'success', onClose: () => setNotification(null) });
            ...
            setNotification({ message, type: 'error', onClose: () => setNotification(null) });
        } finally {
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        try {
            ...
            // Show success message
            setNotification({ message: "Profil berhasil diperbarui! Mengalihkan...", type: 'success', onClose: () => setNotification(null) });
    ...
    const inputClasses = (hasError: boolean, disabled: boolean = false) => (
        `w-full border rounded-md px-4 py-2.5 text-white placeholder-gray-500 transition-colors duration-200
         ...
         ${hasError ? 'border-coc-red focus:border-coc-red focus:ring-coc-red/50' : 'border-coc-gold-dark/50'}`
    );
    ...
    return (
        <main className="container mx-auto p-4 md:p-8 mt-10">
            ...
            <div className="max-w-4xl mx-auto">
                {/* Main profile form */}
                <form onSubmit={handleSubmit} className="card-stone p-6 md:p-8 space-y-10 rounded-lg">
                    {/* Header */}
                            ...
                            {isVerified ? (
                                ...
                                <>
                                    <p className="text-sm text-gray-400 font-sans">
                                        Verifikasi akun Anda untuk mengunci Player Tag & TH Level dengan data real-time dari game, dan membuka fitur manajemen klan.
                                    </p>
                                    ...
                                    <div className="grid grid-cols-1 md:grid-cols-5 gap-4 items-end">
                                        <div className="md:col-span-2">
                                            ...
                                            <FormGroup label="Player Tag" htmlFor="playerTagVerification" error={errors.verifyTag}>
                                                <input id="playerTagVerification" value={verificationForm.playerTag} onChange={handleVerificationChange} placeholder="#P20C8Y9L" className={inputClasses(!!errors.verifyTag, isVerifying)} disabled={isVerifying} />
                                            </FormGroup>
                                        </div>
                                        <div className="md:col-span-2">
                                             {/* Input for API Token */}
                                            <FormGroup label="Token API In-Game" htmlFor="apiTokenVerification" error={errors.verifyToken}>
                                                <input id="apiTokenVerification" value={verificationForm.apiToken} onChange={handleVerificationChange} placeholder="Token dari Pengaturan" className={inputClasses(!!errors.verifyToken, isVerifying)} disabled={isVerifying} />
                                            </FormGroup>
                                        </div>
                                        <div className="md:col-span-1">
                                            {/* Verification Button (type="button") */}
                                            <Button type="button" variant="primary" onClick={handleVerificationSubmit} className="w-full" disabled={isVerifying}>
                                                {isVerifying ? <RefreshCwIcon className="h-5 w-5 animate-spin"/> : <ShieldIcon className="h-5 w-5" />}
                                                <span className="ml-2">{isVerifying ? 'Cek...' : 'Verifikasi'}</span>
                                            </Button>
                                        </div>
                                    </div>
                                </>
                    ...
                    <section className="space-y-4">
                        <h2 className="text-xl font-clash text-coc-gold-dark border-b border-coc-gold-dark/30 pb-2">Pilih Avatar</h2>
                         <div className="flex flex-wrap gap-3 justify-center items-center p-4 bg-coc-stone/30 rounded-lg">
                            {STATIC_AVATARS.map((url) => (
                                <button key={url} type="button" onClick={() => setFormData(prev => ({...prev, avatarUrl: url}))}
                                    className={`relative rounded-full transition-transform duration-200 ${formData.avatarUrl === url ? 'ring-4 ring-coc-green scale-110' : 'ring-2 ring-transparent hover:scale-105'}`}>
                                    <Image src={url} alt="Avatar" width={64} height={64} className="w-16 h-16 rounded-full object-cover border-2 border-coc-gold-dark" />
                    ...
                    <section className="space-y-6">
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                            ...
                            <FormGroup label="Level Town Hall" htmlFor="thLevel" error={errors.thLevel} disabled={isVerified}>
                                <select id="thLevel" value={formData.thLevel} onChange={handleInputChange} className={inputClasses(!!errors.thLevel, isVerified) + ' appearance-none'} disabled={isVerified} required={!isVerified}>
                                    <option value="">-- Pilih TH --</option>
                                    {TH_OPTIONS.map(th => <option key={th} value={th}>Town Hall {th}</option>)}
                                </select>
                            </FormGroup>
                            <FormGroup label="Role Favorit" htmlFor="playStyle">
                                <select id="playStyle" value={formData.playStyle || ''} onChange={handleInputChange} className={inputClasses(false) + ' appearance-none'}>
                                    <option value="">-- Pilih Role --</option>
                                    {PLAY_STYLE_OPTIONS.map(role => <option key={role} value={role}>{role}</option>)}
                                </select>
                            </FormGroup>
