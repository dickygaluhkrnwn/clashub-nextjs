...
// --- TUGAS: Hapus useState, useEffect. Tambah useManagedClanRaid ---
import React, { useState, useMemo, useCallback } from 'react';
// --- TUGAS: Hapus RaidArchive, CocRaidLog. Tambah ManagedClan ---
import { RaidArchive, CocRaidLog, CocRaidMember, ManagedClan, FirestoreDocument } from '@/lib/types'; 
...
import { 
    ...
    ArrowUpIcon, ArrowDownIcon, Loader2Icon, AlertTriangleIcon // <-- TUGAS: Tambah Loader2Icon, AlertTriangleIcon
} from '@/app/components/icons';
import { Button } from '@/app/components/ui/Button';

// --- TIPE SORTIR BARU ---
type RaidSortKey = 'endTime' | 'capitalTotalLoot' | 'totalAttacks' | 'id'; // Ganti 'raidId' ke 'id'
type SortDirection = 'asc' | 'desc';
// -----------------------

// --- TUGAS: Hapus Tipe lokal (sudah diimpor dari types) ---
// interface RaidArchiveWithId extends RaidArchive { ... }

interface RaidTabContentProps {
    clan: ManagedClan;
    // --- TUGAS: Hapus Props Awal ---
    // initialCurrentRaid: CocRaidLog | null | undefined; 
    // initialRaidArchives: RaidArchiveWithId[];
    // onRefresh: () => void;
}
...
// --- TUGAS: Ganti RaidArchiveWithId menjadi FirestoreDocument<RaidArchive> ---
const RaidHistoryItemDetail: React.FC<{ raid: FirestoreDocument<RaidArchive>, isSelected: boolean, onClick: (id: string) => void }> = ({ raid, isSelected, onClick }) => {
    return (
        <div className="border border-coc-gold-dark/30 rounded-lg overflow-hidden bg-coc-dark/20 shadow-sm transition-all duration-300 ease-in-out">
            ...
            >
                <div className="flex items-center space-x-3">
                    ...
                    <div>
                        <p className="font-clash text-coc-gold text-base">Raid Selesai: {formatDate(raid.endTime, false)}</p> {/* TUGAS: Hapus , false */}
...
const RaidTabContent: React.FC<RaidTabContentProps> = ({ clan }) => {
    ...
    // const [loading, setLoading] = useState(false); // <-- DIHAPUS (diganti isLoading)
    // const [currentRaid, setCurrentRaid] = useState<CocRaidLog | null | undefined>(initialCurrentRaid); // <-- DIHAPUS
    // const [raidArchives, setRaidArchives] = useState<RaidArchiveWithId[]>(initialRaidArchives || []); // <-- DIHAPUS
    
    const [selectedArchiveId, setSelectedArchiveId] = useState<string | null>(null);
    const [sort, setSort] = useState<{ key: RaidSortKey, direction: SortDirection }>({ key: 'endTime', direction: 'desc' });


    // --- TUGAS: Hapus useEffect ---
    // useEffect(() => {
    ...
    // --- TUGAS: Ganti handleRefreshClick ---
    const handleRefreshClick = useCallback(() => {
        // setLoading(true); // <-- DIHAPUS
        // onRefresh(); // <-- DIHAPUS
        mutateRaid(); // Panggil mutate dari SWR
    }, [mutateRaid]);
    ...
    const sortedArchives = useMemo(() => {
        // --- TUGAS: Ganti raidArchives (dari state) ke raidArchives (dari hook) ---
    ...
    if (isLoading) {
        return (
            <div className="p-8 text-center bg-coc-stone/40 rounded-lg min-h-[300px] flex flex-col justify-center items-center">
                ...
                <p className="text-lg font-clash text-white">Memuat Data Raid...</p>
                <p className="text-sm text-gray-400 font-sans mt-1">Mengambil data Raid Capital terbaru.</p>
            </div>
        );
    }
    
    // --- TUGAS: Tambah Blok Error ---
    if (error) {
        return (
            <div className="p-8 text-center bg-coc-red/20 rounded-lg min-h-[300px] flex flex-col justify-center items-center">
                ...
                <p className="text-lg font-clash text-white">Error Memuat Data Raid</p>
        ...
        <div className="space-y-6">
            ...
            <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-3">
                ...
                <Button
                    ...
                    disabled={isLoading} // TUGAS: Ganti 'loading' ke 'isLoading'
            ...
            <section aria-labelledby="current-raid-heading">
                ...
                {/* TUGAS: Ganti 'currentRaid' (dari state) ke 'currentRaid' (dari hook) */}
            ...
            <section aria-labelledby="raid-history-heading">
                ...
                {/* TUGAS: Ganti 'raidArchives' (dari state) ke 'sortedArchives' (dari SWR) */}
                {!isLoading && (!sortedArchives || sortedArchives.length === 0) && (
                    <p className="text-gray-400 font-sans italic text-center py-4">Belum ada riwayat raid yang diarsipkan.</p>
                )}
                
                {/* TUGAS: Ganti 'raidArchives.length' ke 'sortedArchives.length' */}
