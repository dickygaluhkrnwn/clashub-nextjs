...
// Menggunakan WarSummary dan WarResult yang diekspor melalui lib/types.ts
...
    onRefresh: () => void;
    // PERBAIKAN: Langsung menggunakan WarSummary dari lib/types.ts
...
interface WarHistoryRowProps {
    war: WarSummary;
    // [BARU] Tambahkan prop untuk menangani klik
    onViewDetails: (warId: string) => void;
}

// PERBAIKAN: WarHistoryRow sekarang menggunakan WarSummary dari lib/types.ts
...
                <Button 
                    size="sm" 
                    // Mengganti "default" (yang tidak valid) dengan "tertiary"
                    variant={hasDetails ? "secondary" : "tertiary"} 
                    disabled={!hasDetails}
                    title={hasDetails ? "Lihat detail serangan dan pemain." : "Hanya ringkasan log tersedia (Data lama)."}
                    className={`text-xs ${!hasDetails ? 'bg-gray-500 hover:bg-gray-600 cursor-not-allowed' : ''}`}
                    // [BARU] Tambahkan onClick untuk membuka modal
...
const WarHistoryTabContent: React.FC<WarHistoryTabContentProps> = ({ clanId, clanTag, onRefresh, initialWarHistory }) => {
    // [BARU] State untuk mengontrol modal: menyimpan ID War yang dipilih
...
    // --- FUNGSI BARU: Buka Modal Detail ---
    ...
    // --- FUNGSI BARU: Tutup Modal Detail ---
...
            {/* [BARU] Tempat untuk War Detail Modal */}
            <WarDetailModal
                warId={selectedWarId}
                clanId={clanId}
