...
const ClanManagementHeader: React.FC<ClanManagementHeaderProps> = ({
  ...
  const handleRefresh = () => {
    ...
    // Menggunakan router.refresh() untuk memicu re-fetch data di server component
    router.refresh();
    // Simulasi delay visual agar user merasakan feedback
    setTimeout(() => setIsRefreshing(false), 1500);
  };

  // Helper untuk Level/Points/Members (Fallback aman)
  // Menggunakan 'any' sementara untuk akses properti API standar jika belum ada di type definition
  ...
  const memberCount = clanAny.membersCount || (Array.isArray(clanAny.members) ? clanAny.members.length : 0);
      ...
      <div className="relative z-10 p-6 md:p-8 flex flex-col md:flex-row gap-6 md:items-center justify-between">
        ...
        <div className="flex flex-col items-center md:items-end gap-4 mt-2 md:mt-0 w-full md:w-auto border-t md:border-t-0 border-white/5 pt-4 md:pt-0">
            ...
            <Button
                ...
                className="w-full md:w-auto backdrop-blur-sm border-white/10 bg-white/5 hover:bg-white/10 text-gray-300 hover:text-white transition-all group"
            >
                <RefreshCwIcon className={`h-4 w-4 mr-2 group-hover:text-coc-gold transition-colors ${isRefreshing ? 'animate-spin text-coc-gold' : ''}`} />
                {isRefreshing ? t.clanManage.processing : 'Refresh Data'}
            </Button>
