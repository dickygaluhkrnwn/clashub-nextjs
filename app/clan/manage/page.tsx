...
const ClanManagementPage = async () => {
  ...
  if (!userProfile?.isVerified || !isClanManager || !userProfile?.clanId) {
    serverError =
      'Akses Ditolak: Anda harus menjadi Leader/Co-Leader yang terverifikasi dan menautkan klan untuk mengakses halaman manajemen.';
    // Tidak perlu redirect, biarkan Client Component menampilkan pesan error
  } else {
    // 3. Ambil data klan dan cache-nya
    try {
      // Kita menggunakan userProfile.clanId, yang diisi saat verifikasi leader/co-leader
      // [FIX] Mengganti userProfile.teamId menjadi userProfile.clanId
      const managedClan = await getManagedClanData(userProfile.clanId);
